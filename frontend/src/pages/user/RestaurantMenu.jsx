...
import { AppContext } from '../../store/AppStore'; // <-- Import AppStore
import CartOverlay from '../../components/CartOverLay';

const RestaurantMenu = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const { 0: storeState } = useContext(AppContext); // <-- Access AppStore state

  // Use allRestaurants from AppStore instead of import
  const restaurant = storeState.restaurants.find(r => r.id === parseInt(id)) || storeState.restaurants[0];
