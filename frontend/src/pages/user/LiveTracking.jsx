...
const LiveTracking = () => {
  ...
  // Correctly mapping to your data.js structure
  ...
  const { orderData, deliveryTimes, distInKm, duration } = useMemo(() => {
    const orderDetails = location.state?.orderDetails || JSON.parse(localStorage.getItem('activeOrder')) || {
      restaurantName: "Express Eats",
      deliveryPosition: officeData.location, // Matches data.js 'location'
      user: { name: "Guest", phone: "0911000000" }
    };

    // Corrected to look for restaurant.location
    const restaurant = allRestaurants.find(r => r.name === orderDetails.restaurantName);
  ...
  }, [location.state, allRestaurants, officeData]);
    ...
    <div className="absolute inset-0 bg-[#1C160E] flex flex-col font-sans overflow-hidden">
      ...
      <div className="relative h-[45%] w-full">
        <div className="w-full h-full z-0">
          <MapContainer 
            key={`${orderData.deliveryPosition.lat}-${orderData.deliveryPosition.lng}`}
